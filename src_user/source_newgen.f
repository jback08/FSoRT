...
*----------------------------------------------------------------------*
      ...
      module source_variables
         ...
         double precision, save :: beam_width_x, beam_width_y
...
*=== Source ===========================================================*
      ...
      double precision FLRNDM, U1, U2, amp, maxGenAmp, Gauss2D
      double precision beamFrac, beam_sigma, beam_offset, beam_mux, beam_muy
      double precision r, min_r, max_r, range_r, dr
      double precision phi, min_phi, max_phi, range_phi, dphi
      double precision sigma_x, sigma_y, mu_x, mu_y
      double precision x, y, beam_mur, beam_rxy
      integer i, j, nr, nphi, accept, nloop
      ...
      if ( first_run ) then
         ...
         option = WHASOU(1)
         beam_sigma = WHASOU(2)
         beam_offset = WHASOU(3)
         min_r = WHASOU(4)
         max_r = WHASOU(5)
         range_r = max_r - min_r
         min_phi = 0.D0
         ! pi constants from include/dlbprc.inc
         max_phi = TWOPIP
         if (option.eq.3) then
            min_phi = -PIHALF
            max_phi = PIHALF
         ...
         KE = SQRT(PBEAM**2 + AM(IJBEAM)**2) - AM(IJBEAM)
         ! Gaussian sigma = FWHM/S2FWHM, S2FWHM = 2*sqrt(2*ln(2)) = 2.355 (3 d.p.)
         beam_width_x = XSPOT/S2FWHM
         beam_width_y = YSPOT/S2FWHM
         ! Gaussian (x, y) mean values & z start
         beam_mux = 0.D0
         beam_muy = 0.D0
         coordinate_z = ZBEAM
         ! Direction cosines
         ...
         ! options: 1 = 100% nominal beam, 2 = wider beam for 1% overlap, 3 = offset beam for 1% overlap
         write(*,*) 'Source option = ', option
         write(*,*) 'Min r = ', min_r, ', max r = ', max_r
         write(*,*) 'Min phi = ', min_phi, ', max phi = ', max_phi
         if (option.eq.2) then
            beam_width_x = beam_sigma
            beam_width_y = beam_sigma
         else if (option.eq.3) then
            beam_mux = beam_offset
            beam_muy = 0.D0
         end if

         ! Beam centre xy radius
         beam_mur = sqrt(beam_mux**2 + beam_muy**2)
         
         ! Maximum Gaussian function value: central peak plus 10%
         maxGenAmp = Gauss2D(0.D0, 0.D0, 0.D0, 0.D0, beam_width_x, beam_width_y)*1.1D0
         write(*,*) 'Central peak = ',maxGenAmp,' for (x,y) = ',coordinate_x,coordinate_y

         if (option.eq.2) then
            ! 2D Gaussian centred on (0, 0) with wider sigma values compared to nominal.
            ! We only need to generate the 2D Gaussian tail. To improve accept/reject efficiency,
            ! set the maximum function value using (x, y) = (min_r, 0.0) plus 10%
            maxGenAmp = Gauss2D(min_r, 0.D0, 0.D0, 0.D0, beam_width_x, beam_width_y)*1.1D0
            
         else if (option.eq.3.and.beam_mur.lt.min_r) then
            ! Case when the beam is offset (nominal sigma) and its peak is inside the baffle inner radius.
            ! We only need to generate the 2D Gaussian tail. To improve accept/reject efficiency,
            ! find the function maximum between r = min_r to max_r and phi = min_phi to max_phi
            maxGenAmp = 0.D0
            nr = 1000
            nphi = 1000
            dr = range_r/(nr*1.0D0)
            dphi = range_phi/(nphi*1.0D0)
            ...
            do i = 1,nr
               ...
               do j = 1,nphi
                  ...
                  amp = Gauss2D(x, y, beam_mux, beam_muy, beam_width_x, beam_width_y)
                  !write(*,*) 'Amp = ',amp,' for r = ',r,', phi = ',phi
                  if (amp.gt.maxGenAmp) then
                     maxGenAmp = amp
                  endif
               enddo
            enddo
            ! Add 10% for accept/reject
            maxGenAmp = maxGenAmp*1.1D0
         endif
            
         write(*,*) 'Updated maxGenAmp = ',maxGenAmp
...
*     options 2 or 3 first, else do option 1
      if (option.eq.2.or.option.eq.3) then
*     Accept/reject method for wider or offset beam.
*     Generate r & phi, get 2D Gaussian amplitude, then compare with the maximum
         accept = 0
         nloop = 0
         do while (accept.eq.0.and.nloop.lt.10000)
            r = sample_flat_distribution(min_r, max_r)
            phi = sample_flat_distribution(min_phi, max_phi)
            x = r*COS(phi)
            y = r*SIN(phi)
            amp = Gauss2D(x, y, beam_mux, beam_muy, beam_width_x, beam_width_y)
            U1 = sample_flat_distribution(0.D0, 1.0D0)
            ! Accept/reject amplitude
            if (U1*maxGenAmp.lt.amp) then
               accept = 1
               coordinate_x = x
               coordinate_y = y
            endif
         ...
         enddo
         if (accept.eq.0) then
            ! We couldn't find a suitable point. Just take one at the minimum baffle radius
            write(*,*) 'Could not find suitable point, so using min_r'
            coordinate_x = min_r
            coordinate_y = 0.D0
         endif
      else
*     Nominal beam
*     Box-Muller transform for 2D (x,y) Gaussian
*     Uniform random numbers used to generate Gaussian
         U1 = sample_flat_distribution(0.D0, 1.0D0)
         U2 = sample_flat_distribution(0.D0, 1.0D0)
         amp = SQRT(-2.0*LOG(U1))
         phi = TWOPIP*U2
         coordinate_x = amp*beam_width_x*COS(Phi) + XBEAM
         coordinate_y = amp*beam_width_y*SIN(Phi) + YBEAM
      endif

*      beam_rxy = sqrt(coordinate_x**2 + coordinate_y**2)
*      write(*,*) 'Gen (x,y,z,r) = ',coordinate_x,coordinate_y,coordinate_z,beam_rxy
*      if (beam_rxy.lt.min_r) then
*         write(*,*) 'beam_rxy < r_min:',beam_rxy,min_r
*      endif
...
*=== End of subroutine Source =========================================*
      ...
      double precision function Gauss2D(x, y, mux, muy, sigx, sigy)
      implicit none
      double precision x, y, mux, muy, sigx, sigy
      double precision ampx, ampy, norm
      double precision PIPIPI, TWOPIP
      parameter ( PIPIPI = 3.141592653589793D+00 )
      parameter ( TWOPIP = 2.D+00 * PIPIPI )

      Gauss2D = 0.D0
      if ((sigx.gt.0.D0).and.(sigy.gt.0.D0)) then
         ampx = exp(-0.5D0*((x - mux)/sigx)**2)
         ampy = exp(-0.5D0*((y - muy)/sigy)**2)
         Gauss2D = ampx*ampy/(TWOPIP*sigx*sigy)
      endif
