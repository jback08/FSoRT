...
# created by the convertUsrbin.py script. Based on getEDensity.py
...
def createRootFile(pars, iFile):
    ...
    rawEVect = ROOT.std.vector('double')()
    scaledEVect = ROOT.std.vector('double')()
        ...
        for inLine in inFile:
            ...
            if gotBins == True and gotAllData == False:
                ...
                for iCol in range(nWords):
                    ...
                    z1Vect.push_back(z1)
                    z2Vect.push_back(z2)
                    rawEVect.push_back(rawE)
                    scaledEVect.push_back(scaledE)
    ...
    if (os.path.isdir('rootFiles') == False):
        os.makedirs('rootFiles')

    rootFileName = 'rootFiles/{0}_{1}_fort{2}.root'.format(pars.targetLabel, fileLabel, iFile)
    print 'Creating {0} from {1}'.format(rootFileName, pars.textFileName)
    ...
    theTree.Branch('rawE', ROOT.addressof(theData, 'rawE'), 'rawE/D')
    theTree.Branch('scaledE', ROOT.addressof(theData, 'scaledE'), 'scaledE/D')
    ...
    for i in range(nTotBins):
        ...
        theData.z1 = z1Vect[i]
        theData.z2 = z2Vect[i]
        theData.rawE = rawEVect[i]
        theData.scaledE = scaledEVect[i]
