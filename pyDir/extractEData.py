...
def createRootFile(pars):
    ...
    for iJob in range(pars.nJobs):
        ...
        evtFileName = '{0}Job{1}/{0}001_evt.txt'.format(pars.targetLabel, iJob)
        ...
        with open(evtFileName, 'r') as evtFile:
            ...
            for inLine in evtFile:
                ...
                if gotLine == True and nSkip == 0:
                    ...
                    nValues = len(words)
                    # Find the number of regions, which is the same for all events 
                    # and data files
                    if gotNRegions == False:

                        theData.nRegions = nValues
                    ...
                    for iR in range(theData.nRegions):
                        ...
                        # Get recursive mean
                        EMean = theData.EDep[iR]
                        theData.EDep[iR] = (N1*EMean + energy)/NEvent

                        # Get recursive standard deviation. We keep the squared terms and 
                        # only take the square root after processing all files & events
                        EDiff = theData.EDep[iR] - energy
                        if N1 > 0.0:
                            theData.EDepErr[iR] = (EDiff*EDiff*NEvent/N1) + theData.EDepErr[iR]

                    # Reset gotLine boolean
                    gotLine = False


    # Finalise standard deviation values
    print 'nRegions = {0}, NEvents = {1}'.format(theData.nRegions, NEvent)
    NEvent1 = NEvent - 1.0
    if NEvent1 > 0.0:

        for iR in range(theData.nRegions):
            theData.EDepErr[iR] = math.sqrt(theData.EDepErr[iR]/NEvent1)
    ...
    for iR in range(theData.nRegions):
        ...
        energy = theData.EDep[iR]
        error = theData.EDepErr[iR]
...
def fillRegionInfo(theData, pars):
    ...
    if 'SingleTarget' in pars.targetLabel:
    ...
    print 'Number of regions = {0}'.format(theData.region.size())
